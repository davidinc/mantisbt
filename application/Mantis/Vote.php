...
Class Mantis_Vote {
     ...
     */
    public static function add( $p_issue_id, $p_weight, $p_user_id = null )
    {
		bug_ensure_exists( $p_issue_id );

        $t_issue_project = bug_get_field( $p_issue_id, 'project_id' );
        $t_vote_max_weight = self::max_weight( $t_issue_project, $p_user_id );
        $t_unlimited = ( $t_vote_max_weight === self::UNLIMITED );
		...
		self::updates_counters_for_issue( $p_issue_id );

        # log vote history
        history_log_event_special( $p_issue_id, BUGVOTE_ADDED, $p_weight );
     ...
     */
    public static function delete( $p_issue_id, $p_user_id )
    {
        if ( $p_issue_id < 1 )
        {
     ...
     * @param integer $p_issue_id issue primary key
     */
    public static function delete_issue_votes( $p_issue_id )
    {
        if ( $p_issue_id < 1 )
        {
            error_parameters( $p_issue_id );
     ...
     * @param integer $p_user_id user primary key
     */
    public static function delete_user_votes( $p_user_id )
    {
        if ( $p_user_id < 1 )
        {
            error_parameters( $p_user_id );
        ...
        $votes = self::get_user_votes( null, true, $p_user_id );
        foreach($votes as $vote)
        {
			self::delete($vote['issue_id'], $p_user_id);
        }
    }
     ...
     * @param integer $p_user_id
     * @return array issues and their weight array('issue_id'=>$p_issue_id, 'weight'=>$p_weight); 
     */
    public static function get_user_votes ($p_project_id = null, $p_include_resolved = true, $p_user_id = null)
    {
        if ($p_user_id === null)
        {
     ...
     */
    public static function get_issue_votes( $p_issue_id )
    {
        if ( $p_issue_id < 1 )
        {
            error_parameters( $p_issue_id );
     ...
     * @return boolean
     */
    public static function is_enabled( $p_project_id = null )
    {
        if ($p_project_id === null)
        {
            $p_project_id = helper_get_current_project();
        }
        $t_enabled = ( ON == config_get( 'voting_enabled', null, null, $p_project_id ) );
        return $t_enabled;
    }
     ...
     * @return boolean
     */
    public static function can_vote( $p_issue_id, $p_user_id = null )
    {
     ...
     * @return boolean
     */
    public static function can_view_details( $p_issue_id, $p_user_id = null )
    {
     ...
     * @return boolean
     */
    public static function exists( $p_issue_id, $p_user_id )
    {
        $t_mantis_bug_votes_table = db_get_table( 'mantis_bug_votes_table' );
        $query = "SELECT COUNT(*)
     ...
     * @return integer
     */
    public static function max_votes( $p_user_id )
    {
     ...
     * note this may also return Mantis_Vote::UNLIMITED so you should always test for this return value
     * 
     * @param integer $p_project_id
     * @param integer $p_user_id
     * @return integer number of available votes or Mantis_Vote::UNLIMITED
     */
    public static function available_votes( $p_project_id = null, $p_user_id = null )
    {
        if ($p_user_id === null) {
            $p_user_id = auth_get_current_user_id();
        }

        $t_max_votes = self::max_votes( $p_user_id );
        ...
        if ($t_max_votes == 0) {
            ...
            $t_used_votes = self::used_votes( $p_project_id, $p_user_id );
     ...
     * @return integer
     */
    public static function used_votes( $p_project_id = null, $p_user_id = null )
    {
        ...
        if ($t_per_project == ON)
        {
            $t_votes = self::get_user_votes( $p_project_id, false, $p_user_id ); 
        }
        else
        {
            $t_votes = self::get_user_votes( ALL_PROJECTS , false, $p_user_id );
        }
     ...
     * @return integer
     */
    public static function max_weight( $p_project_id = null, $p_user_id = null )
    {
        if ($p_project_id === null)
        {
            $p_project_id = helper_get_current_project();
        ...
        $t_available_votes = self::available_votes( $p_project_id, $p_user_id );
...
# return whichever is the lesser, your available votes or you max vote weight
     ...
     * @param integer $p_issue_id
     */
    public static function updates_counters_for_issue( $p_issue_id )
    {
